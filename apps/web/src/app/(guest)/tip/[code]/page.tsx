...
export default function TipPage() {
  ...
  async function handleSubmit(data: TipFormData) {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.clientSecret && stripePromise) {
    ...
    return (
      <div className="flex flex-1 items-center justify-center">
        <div className="text-center">
          <div className="h-10 w-10 animate-spin rounded-full border-2 border-primary/20 border-t-primary mx-auto" />
          <p className="mt-4 text-muted-foreground animate-pulse">Preparing your experience...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 flex-col p-4 pt-6">
      <div className="mx-auto w-full max-w-md space-y-6">
        <StepIndicator currentStep={currentStepNum} totalSteps={4} />
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                {step === 'room' ? (
                  <Button
                    ...
                    className="w-full"
                    onClick={async () => {
                      const valid = await form.trigger('roomNumber');
                      if (valid) setStep('dates');
                    }}
                  >
                    Continue
                  </Button>
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      className="flex-1"
                      onClick={async () => {
                        const valid = await form.trigger(['checkInDate', 'checkOutDate']);
        ...
        {step === 'message' && (
          <div className="space-y-5">
            <div className="text-center">
              <h2 className="text-xl font-bold tracking-tight">Add a Personal Touch</h2>
              <p className="mt-1 text-sm text-muted-foreground">Optional details for your tip</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                onClick={() => handleSubmit(form.getValues() as TipFormData)}
              >
                {form.formState.isSubmitting ? 'Processing...' : 'Next: Payment'}
              </Button>
            </div>
          </div>
        )}
