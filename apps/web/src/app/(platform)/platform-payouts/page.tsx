    ...
    <div className="space-y-8">
      <PageHeader title={t('payoutsTitle')} description={t('payoutsDesc')}>
        <Button onClick={handleProcess} disabled={processing}>
          {processing ? tc('processing') : t('processPayout')}
        </Button>
      </PageHeader>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="card-hover">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-muted-foreground">{tc('completed')}</CardTitle>
          ...
          <CardContent>
            <p className="text-3xl font-bold tracking-tight">
              {formatCurrency(analytics?.totalPaid || 0, undefined, locale)}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {analytics?.paidCount || 0} {t('payoutsTitle').toLowerCase()}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="card-hover">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-muted-foreground">{tc('pending')}</CardTitle>
          ...
          <CardContent>
            <p className="text-3xl font-bold tracking-tight">
              {formatCurrency(analytics?.totalPending || 0, undefined, locale)}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {analytics?.pendingCount || 0} {tc('pending').toLowerCase()}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="card-hover">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-muted-foreground">{tc('failed')}</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {formatCurrency(analytics?.totalFailed || 0, undefined, locale)}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="card-hover">
          ...
          <CardContent>
            <p className="text-3xl font-bold tracking-tight">
              {formatCurrency(analytics?.last30DaysPaid || 0, undefined, locale)}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {payouts.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">
              {t('noPayouts')}. {t('payoutsWillAppear')}
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                    <th className="pb-3 font-medium">{t('staffCount')}</th>
                    <th className="pb-3 font-medium">{t('hotelsTitle')}</th>
                    <th className="pb-3 font-medium">{t('totalRevenue')}</th>
                    <th className="pb-3 font-medium">{t('payoutsTitle')}</th>
                    <th className="pb-3 font-medium">{tc('status')}</th>
                    <th className="pb-3 font-medium">{tc('startDate')}</th>
                    ...
                    <tr key={payout.id} className="border-b last:border-0">
                      ...
                      <td className="py-3">
                        {payout.status === 'failed' && (
                          ...
                          >
                            {retrying === payout.id ? tc('loading') : tc('action')}
                          </Button>
                        )}
                      </td>
                    </tr>
