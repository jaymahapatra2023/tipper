...
export default function PlatformAuditLogPage() {
  ...
  useEffect(() => {
    api.get<FilterOptions>('/platform/audit-logs/filters').then((res) => {
  ...
  const loadLogs = useCallback(async () => {
    ...
    const res = await api.get<AuditLogEntry[]>(`/platform/audit-logs?${params.toString()}`);
  ...
  async function downloadExport() {
    ...
    try {
      ...
      if (action) params.set('action', action);
      if (search) params.set('search', search);
      const blob = await api.downloadBlob(`/platform/audit-logs/export?${params.toString()}`);
